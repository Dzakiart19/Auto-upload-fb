...
- **Simplified Manual Upload Flow** (Updated November 12, 2025):
    - **Previous Flow**: Send media → Enter title → Enter description/hashtags → Process (3 steps)
    - **New Flow**: Send media → Enter title → Auto-process with generated hashtags (2 steps)
    - **Changes**:
      - Removed `awaiting_description` state from Telegram trigger
      - Description field now auto-populated as empty string for manual uploads
      - Hashtags automatically generated by `generateTrendingHashtags` tool based on title
      - Updated processing messages to mention "Generate hashtags otomatis"
    - **Benefits**: Faster workflow, less user input required, consistent hashtag quality
- **Hashtag Duplication Bug Fix** (Fixed November 12, 2025):
    - **Issue**: Hashtags appearing 2-3 times in Facebook posts from URL flow (TikTok/Instagram)
    - **Root Cause**: `prepareMediaData` was concatenating `downloadResult.caption` (already contains hashtags) with `downloadResult.hashtags` (extracted hashtags)
    - **Solution**: Use only `downloadResult.caption` which already includes hashtags naturally from source
    - **Impact**: Clean, single instance of hashtags in Facebook posts for URL flow
...
## Health Check & Monitoring (Added November 12, 2025)
- **Health Endpoint**: `/health` - Returns lightweight JSON (status, uptime, timestamp, service name)
- **Purpose**: Integration with UptimeRobot or other monitoring services for 24/7 uptime monitoring
- **Response Format**:
  ```json
  {
    "status": "healthy",
    "uptime": 123.456,
    "timestamp": "2025-11-12T18:22:31.479Z",
    "service": "telegram-facebook-bot"
  }
  ```
- **Alternative Endpoints**: 
  - `/ping` - Simple keep-alive check
  - `/status` - Full status with public URL and webhook info